...
def create_app(test_config=None):
    ...
    if test_config and 'MONGO_CLIENT' in test_config:
        client = test_config['MONGO_CLIENT']
    else:
        client = MongoClient(f'mongodb://{DB_USERNAME}:{DB_PASSWORD}@{DB_HOST}:27017/finaldb')

    db = client.finaldb
    app.db = db
    app.collection = db.users_auth
    app.expenses_collection = db.user_expenses
    ...
    def calc():
        ...
        if request.method == 'POST':
            ...
            result = app.expenses_collection.insert_one(expenses)
...
if __name__ == '__main__':
    create_app().run(debug=True, host='0.0.0.0', port=35000)
